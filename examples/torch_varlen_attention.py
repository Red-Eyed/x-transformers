...
Compares two approaches on identical weights:
  ...
  - packed: sequences concatenated end-to-end, cu_seqlens passed via flex_attention
...
def check_requirements():
    ...
    if version.parse(torch.__version__) < version.parse("2.5.0"):
        sys.exit(f"PyTorch {torch.__version__} < 2.5.0 – flex_attention not available.")
...
def main():
    ...
    print(f"sequences : {LENGTHS}  (total tokens: {sum(LENGTHS)})")
    ...
    model_pack.load_state_dict(model_pad.state_dict())   # identical weights

    print("padded forward  ...")
    out_pad  = forward_padded(model_pad,  sequences)
    print(f"  output shape: {out_pad.shape}")

    print("packed forward  ...")
    out_pack = forward_packed(model_pack, sequences)
    print(f"  output shape: {out_pack.shape}")
